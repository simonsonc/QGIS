...
void QgsWFSCapabilities::capabilitiesReplyFinished()
{
  ...
  for ( unsigned int i = 0; i < (unsigned)featureTypeList.length(); ++i )
  {
    ...
    for ( unsigned int i = 0; i < (unsigned)otherCRSList.length(); ++i )
    {
    ...
    for ( unsigned int i = 0; i < (unsigned)srsList.length(); ++i )
    {
